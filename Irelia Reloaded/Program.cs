namespace Irelia_Reloaded
{
    ...
    internal class Program
    {
        ...
        {
            if (gapcloser.Sender.LSIsValidTarget() && Menu.Item("gapcloserE").GetValue<bool>() && E.LSIsReady())
            {
        ...
        private static void Combo()
        {
            ...
            if (useWBeforeQ)
            {
                ...
                if (useQ && Q.LSIsReady() && target.LSDistance(Player.ServerPosition) > minQRange)
                {
                    Q.CastOnUnit(target);
                }
            }
            ...
            if (useEStun)
            {
                if (target.CanStunTarget() && useE && E.LSIsReady())
                {
                    E.Cast(target);
                }
            }
            ...
            if (useR && R.LSIsReady() && !UltActivated)
            {
                R.Cast(target);
            }
        }
        ...
        private static float DamageToUnit(AIHeroClient hero)
        {
            ...
            foreach (var spell in spells.Where(x => x.LSIsReady()))
            {
        ...
        private static void DrawingOnOnDraw(EventArgs args)
        {
            ...
            if (drawQ)
            {
                Render.Circle.DrawCircle(p, Q.Range, Q.LSIsReady() ? Color.Aqua : Color.Red);
            }

            if (drawE)
            {
                Render.Circle.DrawCircle(p, E.Range, E.LSIsReady() ? Color.Aqua : Color.Red);
            }

            if (drawR)
            {
                Render.Circle.DrawCircle(p, R.Range, R.LSIsReady() ? Color.Aqua : Color.Red);
            }
        ...
        private static void Harass()
        {
            ...
            if (useW && W.LSIsReady() && Orbwalker.InAutoAttackRange(target))
            {
                W.Cast();
            }
            ...
            Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            if (E.LSIsReady() && E.IsInRange(unit, E.Range) && interruptE)
            {
                E.Cast(unit);
            }

            if (Q.LSIsReady() && E.LSIsReady() && Q.IsInRange(unit, Q.Range) && interruptQe)
            {
                Q.Cast(unit);
        ...
        private static void KillSteal()
        {
            ...
            if (useIgnite && IgniteSlot.LSIsReady())
            {
                var bestTarget =
                    ObjectManager.Get<AIHeroClient>()
        ...
        private static void LastHit()
        {
                ...
                && Environment.TickCount - gatotsuTick >= waitTime * 10)
            {
                foreach (var minion in
                    MinionManager.GetMinions(Q.Range).Where(x => Player.LSGetSpellDamage(x, SpellSlot.Q) > x.Health))
                {
                    if (dontQUnderTower && !minion.LSUnderTurret())
                    {
                        Q.Cast(minion);
                    }
                    else
        ...
        private static void WaveClear()
        {
            ...
            if (useQ && Q.LSIsReady() && Environment.TickCount - gatotsuTick >= waitTime)
            {
                if (useQKillable)
                {
                    var minion =
                        MinionManager.GetMinions(Q.Range)
                            .FirstOrDefault(
                                x => Q.GetDamage(x) > x.Health && (!dontQUnderTower || !x.LSUnderTurret(true)));
            ...
            if ((!useR || !R.LSIsReady())
                && (!R.LSIsReady() || !UltActivated || Player.LSCountEnemiesInRange(R.Range + 100) != 0))
            {
                return;
            }
