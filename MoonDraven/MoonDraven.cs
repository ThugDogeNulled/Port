...
namespace MoonDraven
{
    ...
    internal class MoonDraven
    {
        ...
        {
            if (!this.Menu.Item("UseEGapcloser").IsActive() || !this.E.LSIsReady()
                ...
                || catchOption == 2)
            {
                ...
                if (bestReticle != null && bestReticle.Object.Position.LSDistance(this.Player.ServerPosition) > 100)
                {
                    ...
                    if (this.Menu.Item("DontCatchUnderTurret").IsActive())
                    {
                        ...
                        if (this.Player.LSUnderTurret(true) && bestReticle.Object.Position.LSUnderTurret(true))
                        {
                        ...
                        else if (!bestReticle.Position.LSUnderTurret(true))
                        {
                            if (this.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.None)
                            {
                                EloBuddy.Player.IssueOrder(GameObjectOrder.MoveTo, bestReticle.Position);
        ...
        private void Combo()
        {
            ...
            if (useQ && this.QCount < this.Menu.Item("MaxAxes").GetValue<Slider>().Value - 1 && this.Q.LSIsReady()
                && this.Orbwalker.InAutoAttackRange(target) && !this.Player.Spellbook.IsAutoAttacking)
            {
                this.Q.Cast();
            }

            if (useW && this.W.LSIsReady()
            ...
            if (useE && this.E.LSIsReady())
            {
                this.E.Cast(target);
            }

            if (!useR || !this.R.LSIsReady())
            {
                return;
            }
            ...
            var killableTarget =
                HeroManager.Enemies.Where(x => x.LSIsValidTarget(2000))
                    .FirstOrDefault(
                        ...
                        && (!this.Orbwalker.InAutoAttackRange(x) || this.Player.LSCountEnemiesInRange(this.E.Range) > 2));
        ...
        private void DrawingOnOnDraw(EventArgs args)
        {
            ...
            if (drawE)
            {
                Render.Circle.DrawCircle(
                    ...
                    this.E.Range,
                    this.E.LSIsReady() ? Color.Aqua : Color.Red);
            }
            ...
            if (drawAxeLocation)
            {
                var bestAxe =
                    this.QReticles.Where(
                        x =>
                        x.Position.LSDistance(Game.CursorPos) < this.Menu.Item("CatchAxeRange").GetValue<Slider>().Value)
                        .OrderBy(x => x.Position.LSDistance(this.Player.ServerPosition))
                        .ThenBy(x => x.Position.LSDistance(Game.CursorPos))
                        .FirstOrDefault();
        ...
        private void GameOnOnUpdate(EventArgs args)
        {
            ...
            if (this.W.LSIsReady() && this.Menu.Item("UseWSlow").IsActive() && this.Player.HasBuffOfType(BuffType.Slow))
            {
                this.W.Cast();
            }
        ...
        private void Harass()
        {
            ...
            if (this.Menu.Item("UseEHarass").IsActive() && this.E.LSIsReady())
            {
                this.E.Cast(target);
            }
        }
        ...
        {
            if (!this.Menu.Item("UseEInterrupt").IsActive() || !this.E.LSIsReady() || !sender.LSIsValidTarget(this.E.Range))
            {
                return;
            }
