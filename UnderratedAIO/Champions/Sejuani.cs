    ...
    internal class Sejuani
    {
        ...
        private static void Clear()
        {
            ...
            if (W.LSIsReady() && minionsSpells.Count() > 1 && config.Item("usewC", true).GetValue<bool>() &&
               Player.GetSpell(SpellSlot.W).SData.Mana <= me.Mana)
            {
                W.Cast();
            }
            var minHit = config.Item("useeCmin", true).GetValue<Slider>().Value;
            if (E.LSIsReady() && Player.GetSpell(SpellSlot.Q).SData.Mana <= me.Mana &&
                CombatHelper.SejuaniCountFrostMinion(E.Range) >= minHit &&
                (!(!Q.LSIsReady() && me.Mana - Player.GetSpell(SpellSlot.Q).SData.Mana < me.MaxMana * perc) ||
                 !(!W.LSIsReady() && me.Mana - Player.GetSpell(SpellSlot.W).SData.Mana < me.MaxMana * perc)))
            {
                E.Cast();
            }
            if (config.Item("useqC", true).GetValue<bool>() && Q.LSIsReady() &&
                Player.GetSpell(SpellSlot.Q).SData.Mana <= me.Mana)
            {
        ...
        private static void Combo()
        {
            ...
            {
                if (!(Q.LSIsReady() && me.Mana - Player.GetSpell(SpellSlot.Q).SData.Mana < me.MaxMana * perc) ||
                    !(W.LSIsReady() && me.Mana - Player.GetSpell(SpellSlot.W).SData.Mana < me.MaxMana * perc))
                {
                    E.Cast();
                }
